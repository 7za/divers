...
{
  uint8_t i;
  printf("headers_addr[ehdr = %x, shdr = %x, phdr = %x]\n", ptr->lop_ehdr,
                                                            ptr->lop_shdr,
                                                            ptr->lop_phdr);
...
lop_find_rel_by_sym(struct lop_info *ptr, int sind, int ind)
{
  ...
  for( i = 0; i < numsec; ++i, ++iter){
    if(iter->sh_type == SHT_REL && sind == iter->sh_link){
      ...
      for( j = 0; j < numrel; ++j, ++current){
        if(ELF32_R_SYM(current->r_info) == ind){
          return current; 
        }
      }
    }
...
{
  Elf32_Shdr *dyn = NULL, *str = NULL, *rel = NULL;
  int ret = 0;
...
lop_process_dynsym_section_no_dynrel_error:
  die_now("no relation table for symbol found");
}
